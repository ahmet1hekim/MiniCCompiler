...
AllocaInst* CodeGenerator::createEntryBlockAlloca(Function* func, const std::string& varName, Type* type) {
...
Function* CodeGenerator::codegen(FunctionDecl* func) {
    ...
    for (auto& arg : llvmFunc->args()) {
        ...
        AllocaInst* alloca = createEntryBlockAlloca(llvmFunc, param->getName(), arg.getType());
...
Value* CodeGenerator::codegen(VarDecl* varDecl) {
    ...
    AllocaInst* alloca = createEntryBlockAlloca(currentFunction, varDecl->getName(), type);
...
Value* CodeGenerator::codegen(IfStmt* ifStmt) {
    ...
    cond = builder->CreateICmpNE(cond, ConstantInt::get(*context, APInt(32, 0)), "ifcond");
    
    Function* func = builder->GetInsertBlock()->getParent();
    
    BasicBlock* thenBB = BasicBlock::Create(*context, "then", func);
    BasicBlock* elseBB = ifStmt->getElseBranch() ? BasicBlock::Create(*context, "else") : nullptr;
...
Value* CodeGenerator::codegen(WhileStmt* whileStmt) {
    ...
    cond = builder->CreateICmpNE(cond, ConstantInt::get(*context, APInt(32, 0)), "whilecond");
...
Value* CodeGenerator::codegen(Variable* var) {
    ...
    return builder->CreateLoad(alloca->getAllocatedType(), alloca, var->getName().c_str());
}
...
Value* CodeGenerator::codegen(BinaryExpr* binExpr) {
    ...
    // Comparison operations
        ...
        Value* cmp = isFloat ? builder->CreateFCmpULT(L, R, "cmptmp") : builder->CreateICmpSLT(L, R, "cmptmp");
        return builder->CreateZExt(cmp, Type::getInt32Ty(*context), "booltmp");
    } else if (op == ">") {
        Value* cmp = isFloat ? builder->CreateFCmpUGT(L, R, "cmptmp") : builder->CreateICmpSGT(L, R, "cmptmp");
        return builder->CreateZExt(cmp, Type::getInt32Ty(*context), "booltmp");
    } else if (op == "<=") {
        Value* cmp = isFloat ? builder->CreateFCmpULE(L, R, "cmptmp") : builder->CreateICmpSLE(L, R, "cmptmp");
        return builder->CreateZExt(cmp, Type::getInt32Ty(*context), "booltmp");
    } else if (op == ">=") {
        Value* cmp = isFloat ? builder->CreateFCmpUGE(L, R, "cmptmp") : builder->CreateICmpSGE(L, R, "cmptmp");
        return builder->CreateZExt(cmp, Type::getInt32Ty(*context), "booltmp");
    } else if (op == "==") {
        Value* cmp = isFloat ? builder->CreateFCmpUEQ(L, R, "cmptmp") : builder->CreateICmpEQ(L, R, "cmptmp");
        return builder->CreateZExt(cmp, Type::getInt32Ty(*context), "booltmp");
    } else if (op == "!=") {
        Value* cmp = isFloat ? builder->CreateFCmpUNE(L, R, "cmptmp") : builder->CreateICmpNE(L, R, "cmptmp");
        return builder->CreateZExt(cmp, Type::getInt32Ty(*context), "booltmp");
    }
